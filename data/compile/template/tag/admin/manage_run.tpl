...
<div class="wrap J_check_wrap">
		...
		<div class="nav">
			<ul class="cc">
				<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage'; ?>">话题管理</a></li>
				<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=category'; ?>">话题分类</a></li>
			</ul>
		</div>
		...
		<div class="search_type cc mb10">
			<form action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage'; ?>" method="post">
		...
		<div class="mb10"><a title="添加话题" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=add'; ?>" class="btn J_dialog"><span class="add"></span>添加话题</a></div>
		<?php  if ($tags) { ?>
		...
		<div class="table_list">
			<table width="100%">
				...
				<tr>
					...
					<td><img src="<?php echo htmlspecialchars(Pw::getPath($v['tag_logo']), ENT_QUOTES, 'UTF-8');?>" width="50" height="50" class="fl" onerror="this.onerror=null;this.src='<?php echo Wekit::url()->themes; ?>/site/default/images/tags/default.png'" /></td>
					<td>
						<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>" target="_blank" class="J_tag_name"><?php echo htmlspecialchars($v['tag_name'], ENT_QUOTES, 'UTF-8');?></a>
					...
					<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?id=', rawurlencode($v['tag_id']),'&m=tag&c=manage&a=edit'; ?>" title="编辑话题" class="mr5 J_dialog">[编辑]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>" target="_blank">[内容管理]</a></td>
				</tr>
				<?php  } ?>
				</tbody>
			</table>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=tag&c=manage'; 
...
<?php } ?>
		...
		<div class="btn_wrap">
			<div class="btn_wrap_pd" id="J_sub_wrap">
				...
				<button class="btn J_ajax_submit_btn" data-subcheck="true" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=sethot'; ?>" type="submit">设为热门</button>
				<button class="btn J_ajax_submit_btn" data-subcheck="true" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=deletehot'; ?>" type="submit">取消热门</button>
				...
				<button class="btn J_ajax_submit_btn" data-subcheck="true" data-msg="确定要删除吗？" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=delete'; ?>" type="submit">删除</button>
			</div>
		</div>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
	<?php  } else { ?>
...
Wind.use('dialog', 'ajaxForm', function(){
	var URL_MERGE = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=merge'; ?>",	//合并
		URL_MOVE = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=move'; ?>";		//移动
