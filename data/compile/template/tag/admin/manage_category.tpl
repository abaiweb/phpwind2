...
<div class="wrap">
	<div class="nav">
		<ul class="cc">
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage'; ?>">话题管理</a></li>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=category'; ?>">话题分类</a></li>
		</ul>
	</div>
	...
	<form class="J_ajaxForm" data-role="list" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=setCategory'; ?>" method="post">
		...
		<table width="100%">
			...
			?>
				<tr>
					...
					<td><!-- a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&categoryid=', rawurlencode($value['category_id']); ?>" class="mr5" target="_blank">[访问]</a--><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?id=', rawurlencode($value['category_id']),'&m=tag&c=manage&a=editCategory'; ?>" title="分类编辑" class="mr5 J_dialog">[编辑]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?categoryId=', rawurlencode($value['category_id']),'&m=tag&c=manage'; ?>" class="mr5">[话题管理]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=tag&c=manage&a=deleteCategory'; ?>" class="J_ajax_del" data-pdata="{'id': '<?php echo htmlspecialchars($value['category_id'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></td>
				</tr>
			<?php  } ?>
			</tbody>
			</tbody>
