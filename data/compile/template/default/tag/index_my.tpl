  ...
  foreach($tagContents as $k=>$v){if(!$v) continue; ?>
					<div id="J_tag_item_<?php echo htmlspecialchars($k, ENT_QUOTES, 'UTF-8');?>" class="J_tag_items">
						<div class="hd J_tag_items_hd">
							<a data-id="<?php echo htmlspecialchars($k, ENT_QUOTES, 'UTF-8');?>" class="J_unfollow fr" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=attention&type=del&id=', rawurlencode($k); ?>">取消关注</a>
							<h2><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=view&name=', rawurlencode($myTagsList[$k]['tag_name']); ?>"><?php echo htmlspecialchars($myTagsList[$k]['tag_name'], ENT_QUOTES, 'UTF-8');?> (<?php echo htmlspecialchars($myTagsList[$k]['content_count'], ENT_QUOTES, 'UTF-8');?>)</a></h2>
						</div>
  ...
  foreach($v as $k2=>$v2){ 
							...
							<dl class="cc">
								...
								<dd class="text_content">
									<div class="content">
										<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($v2['created_userid']); ?>" class="name J_user_card_show" data-uid="<?php echo htmlspecialchars($v2['created_userid'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($v2['created_username'], ENT_QUOTES, 'UTF-8');?></a>：
										<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','read.php?tid=', rawurlencode($v2['tid']),'&fid=', rawurlencode($v2['fid']); ?>" target="_blank" class="title"><?php echo htmlspecialchars($v2['subject'], ENT_QUOTES, 'UTF-8');?></a>
										<div class="descrip"><?php echo htmlspecialchars(Pw::substrs(Pw::stripWindCode($v2['content'],true),125), ENT_QUOTES, 'UTF-8');?></div>
									</div>
									<div class="info">
										<span class="time"><?php  echo pw::time2str($v2['created_time'],'auto');  ?></span>&nbsp;来自版块&nbsp;-&nbsp;<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=thread&fid=', rawurlencode($v2['fid']); ?>" target="_blank"><?php echo $v2['forum_name'];?></a>
										<?php  if($relatedTags[$v2['tid']]){ ?>
										<span>
											<?php  foreach($relatedTags[$v2['tid']] as $k3=>$v3){ if ($k3==$k) continue; ?>
											<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=view&name=', rawurlencode($v3['tag_name']); ?>"><?php echo htmlspecialchars($v3['tag_name'], ENT_QUOTES, 'UTF-8');?></a>
											<?php  } ?>
										</span>
										<?php  } ?>
									</div>
  ...
  if ($moreTags) { ?>
						<div class="tag_more_list" id="tag_more_list">
							...
							<?php  foreach($moreTags as $k=>$v){?>
									<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>"><?php echo htmlspecialchars($v['tag_name'], ENT_QUOTES, 'UTF-8');?></a><em>(<?php echo htmlspecialchars($v['content_count'], ENT_QUOTES, 'UTF-8');?>)</em></li>
							<?php  } ?>
							</ul>
						</div>
					<?php  } 
					...
					<div class="not_content">
						啊哦，您还没有关注任何话题哦！试试去<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag'; ?>">关注几个</a>话题！
					</div>
				<?php  } ?>
					</div>
				</div>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=tag&a=my&page=', rawurlencode($_page_i); 
