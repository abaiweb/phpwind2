...
?>
<link href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/article.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
</head>
<body>
<div class="wrap">
				...
				<div class="main_content">
					...
					<nav>
						<div class="content_nav">
							<ul>
								<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=article'; ?>">我的主题</a></li>
								<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=article&a=reply'; ?>">我的回复</a></li>
							</ul>
						</div>
					</nav>
						<?php  if ($posts) { ?>
							...
							<table width="100%" summary="帖子列表" class="mb10">
								...
								<tr>
									<td class="subject">
										...
										<?php if ($value['topped']) {?>
										<!--i class="icon_headtopic_<?php echo htmlspecialchars($value['topped'], ENT_QUOTES, 'UTF-8');?>" title="置顶"></i-->
										<?php }?>
										<i class="icon_topic"></i>
										
										</span>
										<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=read&a=jump&tid=', rawurlencode($value['tid']),'&pid=', rawurlencode($value['pid']); ?>" target="_blank" class="st"><?php echo htmlspecialchars($value['threadSubject'], ENT_QUOTES, 'UTF-8');?></a><?php  if ($value['disabled']) { ?><span class="red">(审核中)</span><?php  } ?>
									</td>
									<td class="tar" width="120">
										<span class="gray"><?php echo htmlspecialchars($value['created_time'], ENT_QUOTES, 'UTF-8');?></span>
									</td>
								</tr>
								<?php  } ?>
							</table>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=article&a=reply&page=', rawurlencode($_page_i); 
...
<?php } ?>
						...
						<?php  } else { ?>
						<div class="not_content">啊哦，您暂没有回复任何主题哦！去<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/',''; ?>">首页看看</a>有什么精彩内容吧！</div>
						<?php  } ?>
					</div>
				</div>
			</div>
